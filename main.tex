...
\vc u = D(x-x_0).
\]
The basic modifier is taken in multiplicative form. Writing $u_i = \lambda_i (x_i-x_{0,i})$, define the one-dimensional factor
\[
\ell_i(u_i)
=
\exp\!\left(
-\frac{1}{d}u_i^2 + \frac{1}{d}\cos(\omega u_i)
\right),
\qquad i=1,\dots,d,
\]
and therefore
\[
\ell(\vc u) = \prod_{i=1}^d \ell_i(u_i),
\qquad \omega > 0.
\]
...
\vc z = Q(x-x_0),
\qquad
D_{\lambda} = \diag(\vc\lambda),
\qquad
\Omega = \diag(\vc\omega).
\]
The generalized modifier is
\[
\ell_{\mathrm{gen}}(\vc z)
=
\exp\!\left(
-\frac{1}{d}\sum_{i=1}^d |(D_{\lambda}\vc z)_i|^{p_i}
+ \frac{1}{d}\vc\kappa^\top \cos(\Omega \vc z)
\right),
\]
...
This is the level of generality that still preserves separability while keeping the meaning of each parameter clear. The vector $\vc\lambda$ controls anisotropic concentration, $\vc p$ controls the marginal tail decay and central sharpness of the one-dimensional factors in the rotated coordinates, $\vc\kappa$ controls the strength of oscillatory multimodality, and $\vc\omega$ controls the oscillation frequency. Because the prior remains isotropic, the role of $Q$ is only to orient the difficult subspace in the ambient coordinates.
...
The implementation can be organized around four parameter blocks: the anisotropy parameters $(D_{\lambda},\vc p,\vc\kappa,\Omega)$, the rotation matrix $Q$, the Gaussian prior parameters $(x_0,s_0)$, and the kernel scales in $R$. In the aligned case the main numerical task is one-dimensional quadrature for a family of integrands of the form
\[
x \mapsto
...
\]
Quadratic coordinates keep the analytically tractable Gaussian-like core, while subquadratic coordinates produce one-dimensional marginal factors with slower tail decay and a sharper peak at the origin. In the inverse-problem interpretation, this does not directly describe the spatial shape of the heterogeneous field, which is controlled by the basis or mode functions encoded through $Q$. Instead, it changes the distribution of the corresponding mode amplitudes: smaller values of $p_i$ increase the probability of both near-zero coefficients and occasional larger excursions, so the associated random field can combine long periods of weak activation with intermittent strong contributions along the affected modes.
%DONE: clarify that the exponents p_i control the distribution of rotated mode amplitudes rather than the spatial shape of the heterogeneous field
